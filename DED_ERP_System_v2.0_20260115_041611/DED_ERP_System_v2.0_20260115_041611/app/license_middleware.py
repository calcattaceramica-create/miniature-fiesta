...
def check_license_validity():
    ...
    if _last_license_check and (now - _last_license_check) < _license_check_interval:
        # Use cached result
        return _license_status['is_valid'], _license_status['message'], _license_status['license']
...
def init_license_middleware(app):
    ...
    def check_license():
        ...
        if not is_valid:
            ...
            return render_license_error(message, license)
...
def render_license_error(message, license=None):
        ...
        <div class="error-container">
            ...
            <div class="info-box">
                <div class="info-row">
                    ...
                    <span class="info-value">{{ license.client_name }}</span>
                </div>
                {% if license.expires_at %}
                <div class="info-row">
                    <span class="info-label">تاريخ الانتهاء - Expiration:</span>
                    <span class="info-value">{{ license.expires_at.strftime('%Y-%m-%d') }}</span>
                </div>
                {% endif %}
                <div class="info-row">
                    <span class="info-label">نوع الترخيص - Type:</span>
                    <span class="info-value">{{ license.license_type }}</span>
                </div>
            </div>
            {% endif %}
    ...
    return render_template_string(template, message=message, license=license), 403
